 ...
 * Please make sure the server is not running, and you close any other
 * application that could be using port 3000. If you do not do this, the tests
 * will not work.
 *
 */
...
describe('add weaknesses', function() {
  var desired = {
    "name" : "Squirtle",
    "weaknesses" : [ "Electric", "Grass", "Ishaan" ]
  };
  pokeDataUtil.restoreOriginalData();
  it('(1 pts.) add weakness "Ishaan" to notapokemon', function(done) {
    request.post(`${URL}/api/weakness/notapokemon/add/Ishaan`,
  ...
  it('(4 pts.) add weakness "Ishaan" to Squirtle', function(done) {
    request.post(
        `${URL}/api/weakness/Squirtle/add/Ishaan`, function(err, res, body) {
          if (err)
            done(err);
          assert.deepEqual(JSON.parse(body), desired);
          var result =
              _.findWhere(pokeDataUtil.loadData().pokemon, {name : 'Squirtle'});
...
describe('delete weaknesses', function() {
  var desired = {"name" : "Squirtle", "weaknesses" : [ "Electric", "Grass" ]};
  it('(1 pts.) delete weakness "Ishaan" from Squirtle (weakness exists)',
     function(done) {
       request.delete(`${URL}/api/weakness/Squirtle/remove/Ishaan`,
                      function(err, res, body) {
                        if (err)
                          done(err);
                        assert.deepEqual(JSON.parse(body), desired);
                        var result =
                            _.findWhere(pokeDataUtil.loadData().pokemon,
                                        {name : 'Squirtle'});
                        assert.deepEqual(result.weaknesses, desired.weaknesses);
                        done();
                      });
     });
  it('(4 pts.) delete weakness "Ishaan" from Squirtle (weakness does not exist)',
     function(done) {
       request.delete(`${URL}/api/weakness/Squirtle/remove/Ishaan`,
                      function(err, res, body) {
                        if (err)
  ...
  it('(4 pts.) delete weakness "Ishaan" from notapokemon', function(done) {
    request.delete(
        `${URL}/api/weakness/notapokemon/remove/Ishaan`,
        function(err, res, body) {
          if (err)
            done(err);
          assert.deepEqual(JSON.parse(body), {});
          assert.deepEqual(
              pokeDataUtil.loadData().pokemon,
              JSON.parse(fs.readFileSync('./poke_original.json')).pokemon);
          done();
        });
  });
});
